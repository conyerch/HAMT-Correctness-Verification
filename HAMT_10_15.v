...
Unset Guard Checking.
    Program Fixpoint getH (hash : list hashKeys) (k : key) (t : table) {struct t} : V :=
      match t with 
...
Set Guard Checking.
  ...
  Theorem empty_relate :
    ...
    Proof. intros. unfold rep_ok. trivial.
    Qed.     
 
    Lemma rep_comm2 : forall (a : key) (v : V) (m : list (key * V)),
    rep_ok (CollisionNode V ((a, v) :: m)) -> rep_ok (CollisionNode V m).
    Proof. intros. simpl. trivial.
    Qed.  

    Lemma map_comm : forall (a k : key) (v : V) (m : list (key * V)),
      map_bound k (map_of_list ((a, v) :: m)) = bIn k ((a, v) :: m) -> 
      map_bound k (map_of_list m) = bIn k m. 
      Proof. intros. unfold map_of_list in H. unfold bIn in H. unfold map_bound in H.
      unfold update in H. unfold t_update in H. destruct (a =? k) eqn: h1. assert ((k =? a) = true).
      rewrite -> eqNat_refl with (b := true). reflexivity. apply h1. rewrite -> H0 in H. simpl in H.
      - induction m.
        * simpl. unfold map_bound. simpl. reflexivity.
        * simpl. unfold update. unfold t_update. unfold map_bound. destruct a0.
        destruct (k =? k0) eqn: h2. rewrite -> eqNat_refl with (b := true).
        reflexivity. apply h2. rewrite -> eqNat_refl with (b := false). unfold map_bound in IHm. apply IHm.
        apply h2.     
      - rewrite -> eqNat_refl with (b := false) in H. unfold map_bound. unfold map_of_list. unfold bIn. unfold update. unfold t_update. apply H. apply h1.
      Qed.                              
  ...
  Theorem lr_help : forall (v : V) (k : key) (map : list (key * HAMT V)) (a l : list (key * V)),
    getH (hash k) k (ListNode V map) = v -> v <> default -> 
    In (k, v) (elements (ListNode V map)).
    Proof. 
    intros. induction map.
    - simpl in H. destruct (hash k) in H. simpl in H0. unfold not in H0. symmetry in H. apply H0 in H. assumption. symmetry in H. unfold not in H0. apply H0 in H. assumption.
    - admit.    
    Admitted.      
   ...
   * simpl. reflexivity.
   * unfold get. simpl. destruct a. apply rep_comm2 in H. apply IHhmes in H. destruct (k =? k0) eqn : h1.                
  ...
  Proof. intros. unfold Abs. induction t.
