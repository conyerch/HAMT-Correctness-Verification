...
Module Type ExtendedTableAbs.
  ...
  Module ExtendedTableHash (VT : ExtendedValType) <: ExtendedTableAbs.
    ...
    Definition hashMapEntry (hash : list nat) :=
      HashMapEntry 
...
Unset Guard Checking.
    ...
    Program Fixpoint set (k : key) (v : V) (t : table) {struct t} : table := 
      ...
      |HashMapEntry _ shift hashF k1 val  => 
        ...
        set k1 val (set k v (ArrayNode V (shift + 1) (HashMapEntry  :: [])))
      | ArrayNode _ shift map => 
        let minKey := getIndex shift (hash k) in 
        if (inList map minKey) then ArrayNode V shift (replace map minKey (set k v (find map k))) else 
        ArrayNode V shift ((minKey, HashMapEntry V shift k v) :: map)
...
Set Guard Checking.
    ...
    Fixpoint allEq (l : list (key * V)) (shift : nat) (test : nat): Prop := 
      match l with 
        |[] => True
        |(k, v) :: t => if ((getIndex shift (hash k)) =? test) then allEq t shift test else False
        end.
    ...
    Definition rep_ok (t : table) : Prop := 
      ...
      | CollisionNode _ shift hmes => 
        match hmes with 
        |[] => True
        |(k, v) :: t => allEq t shift ((getIndex shift (hash k)))
        end
      | Empty _ shift => 
        True 
        end.
